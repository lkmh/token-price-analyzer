...
async def test_get_kline_data_success():
    ...
    mock_response.json = AsyncMock(return_value=[[1, 10, 20, 15, 12, 100, 0, 0, 0, 0, 0, 0]])
...
async def test_get_kline_data_http_error():
    mock_response = AsyncMock()
    mock_response.json.return_value = [] # Set a default return for json()
...
async def test_get_average_price_success():
    mock_response = AsyncMock()
    mock_response.json = AsyncMock(return_value={"mins": 5, "price": "42500.00"})
    mock_response.raise_for_status.return_value = None
