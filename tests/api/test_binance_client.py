...
async def test_get_kline_data_http_error():
    mock_response = AsyncMock()
    mock_response.raise_for_status.side_effect = httpx.HTTPStatusError("Bad Request", request=httpx.Request("GET", "url"), response=httpx.Response(400))
...
async def test_get_kline_data_request_error():
    with patch('httpx.AsyncClient') as mock_async_client:
        mock_async_client.return_value.__aenter__.return_value.get.side_effect = httpx.RequestError("Network Error", request=httpx.Request("GET", "url"))
        data = await get_kline_data("BTCUSDT")
        assert data == []
...
async def test_get_average_price_http_error():
    mock_response = AsyncMock()
    mock_response.raise_for_status.side_effect = httpx.HTTPStatusError("Not Found", request=httpx.Request("GET", "url"), response=httpx.Response(404))
...
async def test_get_average_price_request_error():
    with patch('httpx.AsyncClient') as mock_async_client:
        mock_async_client.return_value.__aenter__.return_value.get.side_effect = httpx.RequestError("Connection Error", request=httpx.Request("GET", "url"))
        data = await get_average_price("BTCUSDT")
        assert data == {}
